...
class SingleImageThumbnailBehavior extends Behavior
{
    ...
    {
        return rtrim(Yii::getAlias($this->destinationPath), '/') . '/' . $type . '-' . $attribute;
    }
    ...
    {
        return rtrim(Yii::getAlias($this->sourcePath), '/') . '/' . $attribute;
    }
    ...
    {
        if ($this->baseUrl) {
            return Url::to(rtrim(Yii::getAlias($this->baseUrl), '/') . '/' . $type . '-' . $attribute);
        }

        $fullPath =  rtrim($this->destinationPath, '/') . '/' . $type . '-' . $attribute;
